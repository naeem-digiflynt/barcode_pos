    ...
    function paypal($id) {
        if ($inv = $this->pay_model->getSaleByID($id)) {
            ...
            if ($paypal->active && (($inv->grand_total - $inv->paid) > 0)) {
                ...
                // redirect('https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=' . $paypal->account_email . '&item_name=' . $inv->reference_no . '&item_number=' . $inv->id . '&image_url=' . base_url() . 'assets/uploads/logos/' . $this->Settings->logo . '&amount=' . (($inv->grand_total - $inv->paid) + $paypal_fee) . '&no_shipping=1&no_note=1&currency_code=' . $this->default_currency->code . '&bn=FC-BuyNow&rm=2&return=' . shop_url('orders/' . $inv->id) . '&cancel_return=' . shop_url('orders/' . $inv->id) . '&notify_url=' . site_url('pay/pipn') . '&custom=' . $inv->reference_no . '__' . ($inv->grand_total - $inv->paid) . '__' . $paypal_fee, 'location');
                redirect('https://www'.(DEMO ? '.sandbox' : '').'.paypal.com/cgi-bin/webscr?cmd=_xclick&business=' . (DEMO ? 'saleem-facilitator@jhj.com' : $paypal->account_email) . '&item_name=' . $inv->reference_no . '&item_number=' . $inv->id . '&image_url=' . base_url() . 'assets/uploads/logos/' . $this->Settings->logo . '&amount=' . (($inv->grand_total - $inv->paid) + $paypal_fee) . '&no_shipping=1&no_note=1&currency_code=' . $this->default_currency->code . '&bn=FC-BuyNow&rm=2&return=' . site_url('pay/pipn') . '&cancel_return=' . site_url('pay/pipn') . '&notify_url=' . site_url('pay/pipn') . '&custom=' . $inv->reference_no . '__' . ($inv->grand_total - $inv->paid) . '__' . $paypal_fee, 'location');
            }
    ...
    function skrill($id) {
        if ($inv = $this->pay_model->getSaleByID($id)) {
            ...
            if ($skrill->active && (($inv->grand_total - $inv->paid) > 0)) {
                ...
                // redirect('https://www.moneybookers.com/app/'.(DEMO ? 'test_' : '').'payment.pl?method=get&pay_to_email=' . (DEMO ? 'demoqco@sun-fish.com' : $skrill->account_email) . '&language=EN&merchant_fields=item_name,item_number&item_name=' . $inv->reference_no . '&item_number=' . $inv->id . '&logo_url=' . base_url('assets/uploads/logos/' . $this->Settings->logo) . '&amount=' . (($inv->grand_total - $inv->paid) + $skrill_fee) . '&return_url=' . site_url('pay/sipn') . '&cancel_url=' . site_url('pay/sipn') . '&detail1_description=' . $inv->reference_no . '&detail1_text=Payment for the sale invoice ' . $inv->reference_no . ': ' . $inv->grand_total . '(+ fee: ' . $skrill_fee . ') = ' . $this->sma->formatMoney($inv->grand_total + $skrill_fee) . '&currency=' . $this->default_currency->code . '&status_url=' . site_url('pay/sipn'));
            }
        ...
        if (!$fp) {
            ...
            while (!feof($fp)) {
                ...
                if (stripos($res, "VERIFIED") !== false) {
                    ...
                    if (($_POST['payment_status'] == 'Completed' || $_POST['payment_status'] == 'Processed' || $_POST['payment_status'] == 'Pending') && ($_POST['business'] == $paypal->account_email || $_POST['business'] == 'saleem-facilitator@jknk.com')) {
